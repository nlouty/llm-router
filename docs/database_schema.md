...
API-key rows are never hard-deleted. Invalidation (and a denied registration) store `is_valid = false` and keep the row as a tombstone, so `requests.user_ip_id` keeps resolving the employee for stats — the partial unique index `uniq_user_ips_active_emp_key` already permits one active plus any number of invalidated rows per employee, so no schema change is involved. Registration checks the key string against every stored row: one owned by the same `employee_no` is revived in place (same id), one owned by a different employee is rejected with HTTP 409, and a fresh string inserts a new row.
...
`session` stores the client session id extracted from the request headers (`x-session-id` and friends; see `router/utils/session.py`), capped at 255 chars, `NULL` when absent. Auto-routing uses it for session-sticky model selection: the most recent committed `router_result` for the session wins with no time bound (issue #313), so a session keeps its model — including a deprecated one that still serves — until that model has no serving servers. This relies on `requests` rows persisting: there is no retention job today, and any future purge would silently cap stickiness at its cutoff. The `idx_requests_session_send` index on `(session, send_time)` backs that lookup and is declared in `RequestRecord._meta.indexes`; `check_db_schema` creates it when missing.
