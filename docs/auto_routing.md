...
6. Query the routing LLM.
   ...
   The choosing request is not sent to an upstream directly. It re-enters the router's own pipeline in-process (`ProxyService.forward_internal`), so it follows the exact same logic as a client request: the active chooser (cluster-aware for prefillers) picks the server, and a chosen prefiller is served through the two-phase prefill → decode PD path. Auto selection is skipped for the internal call, so a routing model with `auto = TRUE` cannot recurse into the choosing algorithm. The whole choosing request is capped at `proxy.llm_choosing_timeout_seconds` (default 10s): each attempt's socket timeouts are clamped to the remaining budget, so a hung routing server is disconnected at the deadline, the choosing record ends as a 504, and the original request falls back to the default model. Choosing-request failures are not counted toward the circuit breaker: a routing server busy prefilling large client requests answers slower than the 10s budget without being unhealthy, and counting those probe timeouts would open its circuit and drop it from every pool. A successful choosing request still records a success and closes an open circuit.
...
This applies to both single-node (`role='mixed'`) servers and PD clusters: in a PD cluster the chosen server is a prefiller, and the prefill probe (`max_tokens=1`) is the first upstream call, so an oversized prompt surfaces as the prefiller's 400 overflow error during prefill. The retry then moves the request to a larger-window prefiller (or mixed server) of the same model. The context window never gates the decode tier: retry candidates come from `list_pd_holders`, which returns only `mixed` and prefiller-role (`prefiller` / `prefix-prefiller`) servers, and operators guarantee `prefillers.context_window + max_completion_tokens < decoders.context_window`. Note that the larger-window candidate query also drops decoders with a smaller `context_window` from the cluster check, so a decoder whose window is set must exceed the failed prefiller's window (or stay `NULL`).

For the overflow detection to engage, `servers.context_window` must be maintained manually and match the server's real limit (e.g. vLLM's `--max-model-len`); a `NULL` (or stale) value disables the retry.
